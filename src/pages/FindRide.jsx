...
export default function FindRide() {
  ...
  return (
    <div className="min-h-screen px-6 pl-20" style={{backgroundColor: '#DEEEED'}}>
      {/* Header */}
      ...
      <div className="flex gap-14">
                ...
                <div className="flex">
                  {[1,2,3,4,5].map(i => (
                    <Star 
                      key={i} 
                      size={16} 
                      className={i <= 4 ? "text-red-500" : "text-gray-300"} 
                      fill={i <= 4 ? "#ef4444" : "#d1d5db"}
                    />
                  ))}
                </div>
              </div>
        ...
        <div className="flex-1 py-4 pr-4">
          ...
          <div className="space-y-4 max-w-3xl">
            {rides.map((ride) => (
              <div key={ride.id} className="rounded-xl p-4 shadow-lg border border-gray-100 hover:shadow-xl transition-shadow w-full" style={{backgroundColor: '#FFFFFF'}}>
                <h3 className="font-bold text-base mb-3">
                  {ride.university} → {ride.location}
                </h3>
                ...
                <p className="text-sm text-gray-600 mb-4 font-normal leading-relaxed">
                  I can pick up from the HUB or the main library. Will be driving a blue Honda Civic.
                </p>
                      ...
                      <div className="flex items-center gap-1">
                        {[1,2,3,4,5].map(i => (
                          <Star 
                            key={i} 
                            size={14} 
                            className={i <= ride.driverRating ? "text-[#E04F5F]" : "text-gray-300"} 
                            fill={i <= ride.driverRating ? "#E04F5F" : "none"}
                          />
                        ))}
                        <span className="text-xs text-gray-500 ml-1 font-normal">({ride.reviewCount} tips)</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    <button 
                      onClick={() => alert(`Booking ride with ${ride.driverName}`)}
                      className="px-6 py-2 bg-black text-white rounded-lg text-sm font-semibold hover:bg-gray-800 transition-colors"
                    >
                      Book Ride
                    </button>
                    <button 
                      onClick={() => alert(`Contacting ${ride.driverName}`)}
